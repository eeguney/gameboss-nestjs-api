...
export class BlogCategoryService {
    ...
    async findAll(
        ...
        if (title) {
            const foundedBlogCategories =
                await this.blogCategoryRepository.find(options);
        ...
        return blogCategories.map((blogCategory) =>
            plainToInstance(BlogCategory, blogCategory),
        );
    }
    ...
    async findById(blogCategoryId: number): Promise<BlogCategory> {
        ...
        if (!blogCategory) {
            throw new NotFoundException(
                `Blog category with id ${blogCategoryId} not found...`,
            );
        }
        return blogCategory;
    }
    ...
    async findByTitle(title: string): Promise<BlogCategory> {
        ...
        if (!blogCategory) {
            throw new NotFoundException(
                `Blog category with id ${title} not found...`,
            );
        }
        return blogCategory;
    }
    ...
    async updateBlogCategory(
        id: number,
        blogCategoryData: UpdateBlogCategoryDto,
    ): Promise<BlogCategory> {
        ...
        if (!blogCategory) {
            throw new NotFoundException(
                `Blog category with id ${id} not found`,
            );
        }
