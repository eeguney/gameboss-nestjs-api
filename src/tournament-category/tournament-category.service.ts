...
async findAll(
    ...
    if (title) {
        ...
        tournamentCategories = await this.tournamentCategoryRepository.find(options);
    }
...
async findById(TournamentCategoryId: number): Promise<TournamentCategory> {
    const TournamentCategory = await this.tournamentCategoryRepository.findOne({
...
async createTournamentCategory(
    ...
    return await this.tournamentCategoryRepository.save(TournamentCategoryData);
}
...
async updateTournamentCategory(
    ...
    const TournamentCategory = await this.tournamentCategoryRepository.findOneBy({
        id,
    });
    if (!TournamentCategory) {
        throw new NotFoundException(
    ...
    this.tournamentCategoryRepository.merge(TournamentCategory, TournamentCategoryData);
